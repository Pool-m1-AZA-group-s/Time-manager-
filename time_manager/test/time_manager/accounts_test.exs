defmodule TimeManager.AccountsTest do
  ...
  describe "clocks" do
    ...
    test "create_clock/1 with valid data creates a clock" do
      valid_attrs = %{status: true, time: ~N[2026-09-21 14:21:00]}

      assert {:ok, %Clock{} = clock} = Accounts.create_clock(valid_attrs)
      assert clock.status == true
      assert clock.time == ~N[2026-09-21 14:21:00]
    end
    ...
    test "update_clock/2 with valid data updates the clock" do
      clock = clock_fixture()
      update_attrs = %{status: false, time: ~N[2026-09-22 14:21:00]}

      assert {:ok, %Clock{} = clock} = Accounts.update_clock(clock, update_attrs)
      assert clock.status == false
      assert clock.time == ~N[2026-09-22 14:21:00]
    end
  ...
  describe "workingtimes" do
    ...
    test "create_working_time/1 with valid data creates a working_time" do
      valid_attrs = %{start: ~N[2026-09-21 14:31:00], end: ~N[2026-09-21 14:31:00]}

      assert {:ok, %WorkingTime{} = working_time} = Accounts.create_working_time(valid_attrs)
      assert working_time.start == ~N[2026-09-21 14:31:00]
      assert working_time.end == ~N[2026-09-21 14:31:00]
    end
    ...
    test "update_working_time/2 with valid data updates the working_time" do
      working_time = working_time_fixture()
      update_attrs = %{start: ~N[2026-09-22 14:31:00], end: ~N[2026-09-22 14:31:00]}

      assert {:ok, %WorkingTime{} = working_time} = Accounts.update_working_time(working_time, update_attrs)
      assert working_time.start == ~N[2026-09-22 14:31:00]
      assert working_time.end == ~N[2026-09-22 14:31:00]
    end
